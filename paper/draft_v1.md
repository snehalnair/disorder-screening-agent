...
Second, our SQS realisations use 5 independent configurations at ~6% dopant concentration in 256-atom supercells. Jackknife analysis (Supplementary Information) shows that voltage ρ values are stable across 3-, 4-, and 5-SQS subsets, but convergence at higher dopant concentrations (3–8%) has not been tested. Lattice Monte Carlo analysis at synthesis temperature (1000 K) reveals that only 38% of dopants (6/16) are truly SQS-compatible (random-like α₁), while 38% self-order and 25% cluster — indicating that the random SQS assumption is an approximation whose accuracy varies by dopant. Grain-boundary segregation and explicit defect-defect correlations beyond nearest-neighbour are not modelled.
