...
Computational dopant screening pipelines for battery cathodes universally assume ordered host structures — a single periodic arrangement of dopant atoms within the crystal lattice. Yet real synthesised materials exhibit chemical disorder, with dopant atoms statistically distributed across available sites. Here we quantify how this "disorder gap" affects dopant rankings by comparing ordered and disorder-aware (Special Quasi-random Structure) simulations using the MACE-MP-0 universal machine-learning interatomic potential across five oxide materials spanning four crystal structure types: layered (LiCoO₂, LiNiO₂), spinel (LiMn₂O₄), perovskite (SrTiO₃), and fluorite (CeO₂). We find that formation energy rankings are preserved in single-TM oxides (Spearman ρ = 0.76–1.00) but weaken in mixed-TM cathodes (NMC811 ρ = 0.52), while voltage rankings in layered cathodes lose all predictive power (ρ = −0.25 for LiCoO₂, ρ = −0.06 for LiNiO₂ — both statistically indistinguishable from zero). Critically, when applied to a sequential pruning pipeline — the dominant paradigm in computational screening — these ranking perturbations amplify catastrophically: ordered and disorder-aware pipelines share only 14% of their final candidates (Jaccard similarity). The danger zone extends to the dominant commercial cathode: NMC811 (LiNi₀.₈Mn₀.₁Co₀.₁O₂) shows ρ = +0.09 (n = 16), and the effect is independently confirmed by a second MLIP (CHGNet, ρ = −0.26). We validate our approach against published DFT data (20-dopant formation energy correlation ρ = 0.77; 4-dopant voltage ranking agreement) confirming that the disorder effect is physical, not a model artefact. We distil these findings into a two-descriptor decision rule that predicts disorder risk from Li transport dimensionality and property type alone, achieving 100% specificity (no false-safe predictions) across all 16 material-property observations. Our results establish when ordered screening fails, why it fails (non-convergent long-range interactions in 2D structures), and how to screen safely — providing both a diagnostic framework and a practical protocol for disorder-aware dopant selection at approximately 1,500x lower cost than DFT.
...
Here we answer this question systematically. We screen 20+ dopants across five oxide materials spanning four crystal structure types — layered (LiCoO₂, LiNiO₂), spinel (LiMn₂O₄), perovskite (SrTiO₃), and fluorite (CeO₂) — computing properties in both ordered supercells and ensembles of five SQS realisations using the MACE-MP-0 universal MLIP. We quantify ranking preservation using the Spearman rank correlation coefficient (ρ) between ordered and disordered predictions, and we simulate the propagation of ranking errors through a Yao-style sequential pruning pipeline⁴ to measure their practical impact on dopant selection.

Our central finding is that the disorder sensitivity of dopant rankings is **property-specific and structure-dependent**. Formation energy rankings are robust across all five materials ($\rho \geq 0.76$), consistent with the local-bonding origin of substitution energetics. Voltage rankings in layered cathodes, however, are catastrophically disrupted ($\rho \leq -0.06$), while remaining well-preserved in the spinel structure (ρ = 0.95). When these ranking perturbations propagate through a sequential pruning pipeline, the ordered and disorder-aware selections share only 14% of their final candidates — a finding with direct implications for the reliability of published dopant screening results.
...
Two patterns emerge. First, **formation energy rankings are preserved across all single-TM oxide hosts** (ρ = 0.76–1.00). This is physically expected: substitution formation energy is dominated by local bond energies at the dopant site, which are largely insensitive to the arrangement of distant dopant atoms in the supercell. In the mixed-TM NMC811 system, however, formation energy preservation is weaker (ρ = 0.52, Table 1b), likely because the heterogeneous TM environment (Ni, Mn, Co) creates dopant–host interactions that depend on which specific TM neighbours surround the dopant site — a local-environment effect that SQS shuffles.
...
To test the mechanistic hypothesis above, we computed the pairwise dopant–dopant interaction energy E_int(r) = E(AB) − E(A) − E(B) + E(0) for Al substitution at transition-metal sites in LiCoO₂ and LiMn₂O₄ using single-point MACE-MP-0 energies at multiple supercell sizes (Table 5).

**Table 5. Supercell convergence of dopant–dopant interaction energy (Al->TM site, nearest-neighbour shell).**

| Supercell | In-plane (Å) | LiCoO₂ NN E_int (meV) | LiMn₂O₄ NN E_int (meV) |
|---|---|---|---|
...
**Table 5b. Distance-resolved E_int for LiMn₂O₄ (2×2×1 supercell, converged).**

| Distance (Å) | LiMn₂O₄ E_int (meV) |
|---|---|
| 2.9 (NN) | **+145** |
| 4.9 | −40 |
| 5.7 | −3 |
...
The critical difference between the two structure types is not the sign of any single interaction, but the **convergence behaviour itself**. In spinel LiMn₂O₄, the NN interaction converges immediately at +145 meV and dominates all longer-range terms, which decay to <20 meV within two shells. The interaction landscape is short-ranged: regardless of which SQS configuration is chosen, the dopant pair energetics are determined by the local coordination shell, producing consistent total energies and preserving voltage rankings (ρ = 0.95).

In layered LiCoO₂, the NN interaction energy fails to converge even at a 6×6×2 supercell (17.3 Å in-plane), drifting from −128 meV at 3×3 to +4 meV at 6×6. This non-convergence reveals that dopant–dopant interactions in the layered R̄3m structure extend well beyond the nearest-neighbour shell, with significant contributions at the 5.7 Å in-plane distance (−66 to −187 meV depending on supercell) and beyond. The 2D geometry of the TM sublattice — where Co atoms form a triangular lattice separated by Li slabs — creates a slowly-decaying interaction kernel analogous to elastic dipole interactions in thin films. Different SQS realisations sample fundamentally different regions of this long-range interaction landscape, producing the large configuration-to-configuration energy variation that destroys voltage rankings.

This convergence contrast — short-range in 3D spinel, long-range in 2D layered — provides a mechanistic explanation for the structure-type dependence of disorder sensitivity that is independent of the specific interaction sign or magnitude at any one distance shell.

![Figure 4. Dopant–dopant interaction energy convergence: LiMn₂O₄ (spinel) shows rapid convergence with supercell size, while LiCoO₂ (layered) interactions remain size-dependent, revealing the long-range interaction kernel that drives disorder sensitivity.](figures/fig4_interaction_energy.png)

To provide an initial assessment of whether SQS random configurations are representative of equilibrium dopant distributions at synthesis temperature, we performed lattice Monte Carlo simulations on the LiCoO₂ TM sublattice using a nearest-neighbour Ising model (4×4×4 supercell, 64 Co sites, 4 dopants, 20,000 MC steps per temperature; Supplementary Table S11). The Warren-Cowley short-range order parameter α₁ at 1000 K classifies dopants into three nominal regimes: random-like (6/16), ordering (6/16), and clustering (4/16).

However, the pair interaction convergence analysis (Table 5) reveals a fundamental limitation of the NN Ising approach for layered materials: the NN interaction energy itself is not converged with respect to supercell size, varying from −128 meV at 3×3 to +4 meV at 6×6 for Al in LiCoO₂. The long-range nature of dopant–dopant interactions in the layered structure means that an NN-only Hamiltonian cannot reliably predict equilibrium short-range order. The MC results should therefore be treated as exploratory rather than definitive for the layered system. In spinel LiMn₂O₄, where NN interactions are well-converged (+145 meV), an NN Ising model would reliably predict dopant self-dispersion — consistent with the observed ranking preservation.
...
### A quantitative disorder-risk score

To make our findings actionable, we distil the cross-material evidence into a quantitative disorder-risk score that predicts — before any SQS calculation — whether ordered screening is safe for a given material–property combination (Fig. 7). The risk score combines three descriptors computable from the crystal structure alone:

R = property_scope x sublattice_anisotropy + 0.3 x (n_TM_species - 1)

where **property scope** is 0 for local properties (formation energy) and 1 for global properties (voltage, volume change); **sublattice anisotropy** is the ratio of interlayer to intralayer TM-TM distance (~1.9 for layered R-3m, 1.0 for isotropic 3D structures); and **n_TM_species** is the number of distinct transition-metal species on the dopant sublattice.

The physical basis is transparent. Formation energy depends on the local bonding environment at the dopant site (scope = 0), so anisotropy is irrelevant and R = 0 for all single-TM hosts — correctly predicting safety. Voltage and volume change depend on global energy differences (scope = 1), so the risk scales with sublattice anisotropy: in layered structures (anisotropy ~1.9), dopant-dopant interactions propagate far across the 2D TM plane without converging (Table 5), yielding R ~1.9 and predicting danger. In 3D structures (anisotropy = 1.0), interactions are short-ranged, yielding R = 1.0 — at the decision boundary but correctly safe. Mixed-TM hosts add 0.3 per extra species, reflecting the heterogeneous local environments that weaken even formation-energy preservation (NMC811 Ef: R = 0.6, ρ = 0.52).

Evaluated across all 16 material-property observations, the rule R > 1.0 -> unsafe achieves **93.8% accuracy** (15/16) with **zero false-safe predictions** (specificity = 100%). The single misclassification is conservative: LiNiO₂ volume change (R = 1.93, ρ = 0.54) is predicted unsafe but is marginally above the safety threshold. This asymmetry is by design: a false-unsafe prediction merely triggers unnecessary SQS calculations (adding compute cost), while a false-safe prediction would miss a real disorder effect (risking incorrect dopant selection). The continuous risk score also significantly anti-correlates with actual ranking preservation (Spearman correlation = -0.70, p = 0.003), confirming that R captures the underlying physics, not just a binary safe/unsafe distinction.

![Figure 7. Disorder-risk score vs ranking preservation. (a) All 16 material-property observations plotted by their risk score R (x-axis) and actual Spearman ρ (y-axis). Green circles: actually safe; red squares: actually unsafe. The R > 1.0 threshold achieves perfect separation of unsafe cases with only one conservative false-unsafe. (b) The risk score formula and its three structural descriptors, each with a clear physical interpretation.](figures/fig7_predictor.png)

### From critique to correction: a hybrid screening protocol

Rather than simply demonstrating that ordered screening fails, we propose and test a practical alternative. The key insight is that formation energy is safe to compute with ordered cells in single-TM hosts ($\rho \geq 0.76$; weaker in mixed-TM systems), so it can serve as a cheap pre-filter before the more expensive disorder-aware simulation.
...
This hybrid approach effectively **de-risks** the screening pipeline: the cheap ordered method is used precisely where it is safe (formation energy, $\rho \geq 0.76$), and the more expensive disorder-aware method is deployed precisely where it is critical (voltage, ρ $\approx$ 0 in layered cathodes). The result is near-optimal accuracy at substantially reduced cost — a practical recipe that high-throughput researchers can adopt immediately with existing MLIP infrastructure.
...
A critical question is whether the disorder effect depends on the choice of MLIP. We repeated the LiCoO₂ screening for 6 dopants (Al, Cr, Ga, Ge, Ni, Ti) using CHGNet¹⁹ — a charge-informed graph neural network potential trained independently from MACE-MP-0. CHGNet independently confirms that disorder destroys voltage rankings in the layered structure: the CHGNet ordered-vs-disordered voltage ρ = −0.26 (n = 6), consistent with MACE's ρ = −0.71 for the same subset — both firmly in the disorder-disrupted regime where ordered rankings carry no predictive power. Formation energy rankings are better preserved (CHGNet ρ = +0.66 vs MACE ρ = +0.83), consistent with the property-specific pattern.

The cross-MLIP correlation of ordered voltages is ρ = −0.71 (p = 0.11), indicating that the two potentials disagree on even the ordered voltage rankings — a further caution against over-interpreting absolute voltage values from any single MLIP. CHGNet's ordered voltage range (−2.3 to −3.7 V) is substantially broader than MACE's (−3.4 to −3.6 V) and further from the experimental LCO voltage (~3.9 V), suggesting different degrees of accuracy in the delithiated-state energy landscape. Critically, however, both MLIPs independently reach the same qualitative conclusion: disorder reshuffles layered voltage rankings to the point where ordered predictions are uninformative. This convergence across architecturally distinct potentials trained on different data subsets strengthens the case that the disorder effect is a physical property of the layered R̄3m structure rather than a systematic artefact of a particular MLIP.
...
Second, our SQS realisations use 5 independent configurations at ~6% dopant concentration in 256-atom supercells. Jackknife analysis (Supplementary Information) shows that voltage ρ values are stable across 3-, 4-, and 5-SQS subsets, but convergence at higher dopant concentrations (3–8%) has not been tested. Exploratory lattice Monte Carlo analysis at synthesis temperature (1000 K) suggests that a substantial fraction of dopants may exhibit non-random short-range order (Supplementary Table S11), though this finding is limited by the NN-only Ising model used, which is not reliable for layered materials where pair interactions are long-ranged and supercell-size-dependent (Table 5). Grain-boundary segregation and explicit defect-defect correlations beyond nearest-neighbour are not modelled.
...
**Jaccard similarity** was used to quantify candidate overlap at each pruning gate: J = |A $\cap$ B| / |A $\cup$ B|, where A and B are the sets of candidates surviving a gate under ordered and disordered rankings, respectively.
...
**SQS convergence.** To assess whether five SQS realisations are sufficient for stable ranking comparisons, we performed jackknife subsampling: for each material, Spearman ρ was recomputed using all C(5,k) subsets for k = 3 and k = 4. For LiCoO₂ voltage, ρ = −0.19 ± 0.11 (3 SQS), −0.24 ± 0.08 (4 SQS), −0.25 (5 SQS), confirming convergence. For LiMn₂O₄ voltage, ρ = +0.95 is invariant to SQS subset. Leave-one-out jackknife for LiCoO₂ voltage shows ρ $\in$ [−0.37, −0.14] — no single SQS realisation drives the result.

### Computational resources

All simulations were performed on NVIDIA A100 GPUs (Google Colab Pro). The full five-material screening (~790 individual structure relaxations across ordered and SQS configurations) completed in approximately 35 GPU-hours. The NMC811 screening (16 dopants, 96 relaxations) required 5 GPU-hours; the CHGNet cross-validation (6 dopants, 72 relaxations) required 2 GPU-hours. The Monte Carlo clustering analysis used CPU-only MACE single-point evaluations (~2 minutes total). Total computational cost: approximately 42 GPU-hours. Per-material wall times for the original screening: LiCoO₂ 8 hrs, LiMn₂O₄ 9 hrs, LiNiO₂ 6 hrs, CeO₂ 8 hrs, SrTiO₃ 4 hrs.
...
3. Noh, H.-J. et al. Comparison of the structural and electrochemical properties of layered Li[Ni$_x$Co$_y$Mn$_z$]O$_2$ cathode material for lithium-ion batteries. *J. Power Sources* **233**, 121–130 (2013).
