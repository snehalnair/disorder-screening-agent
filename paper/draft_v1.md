...
To assess whether SQS random configurations are representative of equilibrium dopant distributions at synthesis temperature, we performed lattice Monte Carlo simulations on the LiCoO₂ TM sublattice for all 16 dopants (4×4×4 supercell, 64 Co sites, 4 dopants, 20,000 MC steps per temperature). The Warren-Cowley short-range order parameter α₁ at 1000 K classifies dopants into three regimes:

- **Random-like** (|α₁| < 0.05): 6/16 dopants (Al, Ti, Mg, Fe, W, Ge) — SQS assumption is physically justified.
- **Ordering** (α₁ < −0.05): 6/16 dopants (Zr, Cr, Mn, V, Mo, Sn) — repulsive NN interactions drive self-dispersion; the ordered model may better approximate the equilibrium distribution than random SQS.
- **Clustering** (α₁ > +0.05): 4/16 dopants (Nb, Ga, Ni, Cu) — attractive NN interactions drive aggregation; neither ordered nor random SQS captures the equilibrium configuration, and the true disorder gap may be larger than our SQS estimate.

This three-way split has important implications: for the ordering group, our SQS-based disorder gap is an upper bound (the real material self-orders), while for the clustering group, it is a lower bound (the real material is more clustered than random SQS). Notably, the clustering group includes Ni — a key dopant in NMC cathodes — suggesting that the disorder sensitivity of NMC materials may be even more pronounced than our random-SQS analysis indicates.
